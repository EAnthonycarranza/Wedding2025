...
const BottomNavigationBar = ({
  ...
  fileInputRef,
  isOpen,
  profileImageUrl, // This should be passed from UploadPage.js
}) => {
  const [showMoreMenu, setShowMoreMenu] = useState(false);

  console.log("Rendering BottomNavigationBar. Received profileImageUrl: ", profileImageUrl); // Log the profile image URL
      ...
      {!showMoreMenu ? (
        ...
        >
          <BottomNavigationAction
            ...
            sx={{ color: bottomNavValue === 'gallery' ? '#5B6ACF' : 'black' }}
          />
          <BottomNavigationAction
            label="Voicemail"
            value="voicemail"
            icon={<VoicemailIcon sx={{ fontSize: 28 }} />}
            sx={{ color: bottomNavValue === 'voicemail' ? 'black' : 'black' }}
          />
          <BottomNavigationAction
            label="Upload"
            value="upload"
            icon={<AddCircleIcon sx={{ fontSize: 28 }} />}
            sx={{ color: bottomNavValue === 'upload' ? 'black' : 'black' }}
          />
          
          {/* User's Google Profile Avatar */}
          <BottomNavigationAction
            label="Profile"
            value="profile"
            icon={
              <Avatar
                alt="User Profile"
                src={profileImageUrl || '/path/to/placeholder/avatar.png'} // Placeholder if no image
        ...
        <BottomNavigation
          onChange={handleBottomNavChange}
          showLabels
          sx={{ height: '60px', backgroundColor: 'white', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)' }}
        >
          <BottomNavigationAction
            label="Go Back"
            value="back"
